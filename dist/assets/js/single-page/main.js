...
d3.json("assets/json/paintings.json", function (paintingData) {
    ...
    if (loaded) {
      if (state.activePainting === undefined) {
        ...
        if (state.activeTour === undefined) {
          ...
          zoom.translateExtent([[anchorBounds.x - translateExtentTourBuffer, anchorBounds.y - translateExtentTourBuffer], [anchorBounds.x + paintingBounds.width + translateExtentTourBuffer, anchorBounds.y + paintingBounds.height + translateExtentTourBuffer]]);
        }
  ...
  function renderPaintingTour(paintingTourSelection) {
    ...
    if (state.activePainting !== undefined && state.activeTour !== undefined) {
      ...
      //Tour Bounds
      var tourBounds = allPaintingTours.select(".bounds-container").selectAll("rect").data(function (d, i) {
        if (stepIsActive(d, i)) {
          return d.bounds;
        } else {
          return [];
